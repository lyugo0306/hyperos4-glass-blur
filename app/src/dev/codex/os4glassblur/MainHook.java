...
public final class MainHook extends XposedModule {
    ...
    static final String CONFIG_KEY = "os4_glass_blur_config";

    // ---- runtime-tunable config (defaults; overridable via SettingsActivity) ----
    private int glassRadius = 40;
    private int panelBlurPercent = 50;
    private float deltaBrightness = -0.05f;
    private float deltaDarker = -0.08f;
    private float deltaIor = 0.20f;
    private float deltaBurn = -0.25f;
    // advanced tunables (indices per MiBackgroundStyle.calculateGlassParams)
    private float deltaSaturation = 0f;       // [5]
    private float deltaAlpha = 0f;            // [14]
    private float deltaTintR = 0f;            // [11]
    private float deltaTintG = 0f;            // [12]
    private float deltaTintB = 0f;            // [13]
    private float deltaEdgeThickness = 0f;    // [21]
    private float deltaReflectStrength = 0f;  // [24]
    private float deltaLightIntensity = 0f;   // [28]
    private float deltaBgSat = 0f;            // [33]
    private float deltaBgBright = 0f;         // [34]
    private boolean rowGlassEnabled = true;
    private boolean sdfClampEnabled = true;
    private float panelScaleStrength = 1.0f;  // shade background zoom (0=off, 2=double)
    ...
    private final java.util.Map<View, Boolean> forcedRows =
    ...
    public void onModuleLoaded(ModuleLoadedParam param) {
        ...
        // Early in process start ActivityThread.currentApplication() may be
        // null, which silently disables config + observer; retry until the
        // app context is available (max ~10s).
        final java.util.concurrent.atomic.AtomicInteger tries =
                new java.util.concurrent.atomic.AtomicInteger(0);
        new Thread(new Runnable() {
            @Override
            public void run() {
                while (tries.incrementAndGet() <= 5 && getAppContext() == null) {
                    try {
                        Thread.sleep(2000);
    ...
    /** Read tunables from Settings.System (written by the settings page). */
    private void loadConfig() {
        try {
            ...
            String json = android.provider.Settings.System.getString(
                    ctx.getContentResolver(), CONFIG_KEY);
            if (json == null || json.trim().isEmpty()) {
                return;
            }
            org.json.JSONObject o = new org.json.JSONObject(json);
            glassRadius = clampInt(o.optInt("radius", 40), 0, 500);
            panelBlurPercent = clampInt(o.optInt("panelBlur", 50), 0, 100);
            deltaBrightness = (float) o.optDouble("brightness", -0.05);
            deltaDarker = (float) o.optDouble("darker", -0.08);
            deltaIor = (float) o.optDouble("ior", 0.20);
            deltaBurn = (float) o.optDouble("burn", -0.25);
            deltaSaturation = (float) o.optDouble("saturation", 0);
            deltaAlpha = (float) o.optDouble("alpha", 0);
            deltaTintR = (float) o.optDouble("tintR", 0);
            deltaTintG = (float) o.optDouble("tintG", 0);
            deltaTintB = (float) o.optDouble("tintB", 0);
            deltaEdgeThickness = (float) o.optDouble("edgeThickness", 0);
            deltaReflectStrength = (float) o.optDouble("reflectStrength", 0);
            deltaLightIntensity = (float) o.optDouble("lightIntensity", 0);
            deltaBgSat = (float) o.optDouble("bgSat", 0);
            deltaBgBright = (float) o.optDouble("bgBright", 0);
            rowGlassEnabled = o.optBoolean("rowGlass", true);
            sdfClampEnabled = o.optBoolean("sdfClamp", true);
            panelScaleStrength = (float) o.optDouble("panelScale", 1.0);
            notificationRecipe = buildNotificationRecipe();
            configLoaded = true;
            Log.i(TAG, "config loaded: radius=" + glassRadius + " panelBlur="
                    + panelBlurPercent + " brightness=" + deltaBrightness
                    + " darker=" + deltaDarker + " ior=" + deltaIor
                    + " burn=" + deltaBurn + " sat=" + deltaSaturation
                    + " alpha=" + deltaAlpha + " tint=" + deltaTintR + "/"
                    + deltaTintG + "/" + deltaTintB + " edge=" + deltaEdgeThickness
                    + " refl=" + deltaReflectStrength + " light="
                    + deltaLightIntensity + " bgSat=" + deltaBgSat
                    + " bgBright=" + deltaBgBright
                    + " rowGlass=" + rowGlassEnabled
                    + " sdfClamp=" + sdfClampEnabled
                    + " panelScale=" + panelScaleStrength);
        } catch (Throwable t) {
            Log.e(TAG, "loadConfig failed", t);
        }
    }

    /** Re-apply config when the settings page saves (no reboot needed for
     *  most paths; rows re-run their pipeline on the next layout). */
    private void registerConfigObserver() {
        try {
            final android.content.Context ctx = getAppContext();
            if (ctx == null) {
                return;
            }
            ...
            ctx.getContentResolver().registerContentObserver(
                    android.provider.Settings.System.getUriFor(CONFIG_KEY),
                    false, observer);
        } catch (Throwable t) {
            Log.e(TAG, "registerConfigObserver failed", t);
        }
    }
    ...
    /**
     * Application context from ActivityThread (libxposed has no getContext).
     * NOTE: must be the real application context (package matches the caller
     * uid); getSystemContext() would report package "android" and
     * Settings.System reads would throw SecurityException.
     */
    ...
    private void installHooks() {
        ...
        try {
            ...
            try {
                ...
                hook(onAttach).intercept(chain -> {
                    try {
                        ...
                        if (rowGlassEnabled
                                && !Boolean.TRUE.equals(forcingState.get())
                ...
                try {
                    if (!Boolean.TRUE.equals(forcingState.get())) {
                        ...
                        if (original != null && original.length >= 36) {
                            // Heads-up notifications use their own glass
                            // array (alpha=1.5, IOR=3.0, reflect 600...);
                            // clicking the HUN switches to the list array ->
                            // visible color/transparency jump. Replace HUN
                            // arrays with the unified list recipe.
                            if (notif && isHeadsUpArray(original)) {
                ...
                if (isControlCenterCall()) {
                    if (!glassLogged) {
                        ...
                        Log.i(TAG, "glass-card radius " + chain.getArg(0) + "/"
                                + chain.getArg(1) + " -> " + glassRadius + "/" + glassRadius
                                + " view=" + cls);
                    }
                    return chain.proceed(new Object[] { glassRadius, glassRadius });
                }
            ...
            try {
                ...
                hook(setMiBackgroundBlurRadius).intercept(chain -> {
                    if (isShadeBlurProviderCall()) {
                        ...
                        int adjusted = Math.round(original * (panelBlurPercent / 100.0f));
                    ...
                    if (stackContains("BlurUtilsImpl", "applyModalContainerBlur")) {
                        return chain.proceed(new Object[] { glassRadius });
                    }
                    return chain.proceed();
                });
            } catch (Throwable ignored) {
            ...
            // ---------- panel background scale: blur scale ratio ----------
            // ShadeBlendBlurController scales the shade background
            // (BlurProviderView) while pulling:
            //   view.setMiBackgroundBlurScaleRatio(lerp(0, MAX_SCALE_RADIUS,
            //   blurRatio))   (MAX_SCALE_RADIUS = 0.075)
            // Remap the ratio: strength=0 -> no background zoom,
            // 1 -> stock, 2 -> doubled.
            try {
                final Method setScaleRatio = android.view.View.class.getMethod(
                        "setMiBackgroundBlurScaleRatio", float.class);
                hook(setScaleRatio).intercept(chain -> {
                    if (panelScaleStrength != 1.0f) {
                        float r = ((Number) chain.getArg(0)).floatValue();
                        return chain.proceed(new Object[] {
                                r * panelScaleStrength });
                    }
                    return chain.proceed();
                });
                Log.i(TAG, "blurScaleRatio hook installed");
            } catch (Throwable t) {
                Log.e(TAG, "blurScaleRatio hook failed: " + t);
            }
    ...
    private void forceGlass(Object view, String via) {
        try {
            ...
            if (viewSetGlassRadius != null && isRowBackground(v)) {
                viewSetGlassRadius.invoke(v, glassRadius, glassRadius);
            }
            ...
            synchronized (forcedRows) {
                forcedRows.put(v, Boolean.TRUE);
            }
    ...
    /** Re-apply glass on every row we ever forced, after a config change. */
    private void refreshForcedRows() {
        try {
            final java.util.List<View> rows;
            synchronized (forcedRows) {
                rows = new java.util.ArrayList<View>(forcedRows.keySet());
            }
            if (rows.isEmpty()) {
                return;
            }
            ...
            main.post(new Runnable() {
                ...
                public void run() {
                    for (View v : rows) {
                        if (v.isAttachedToWindow()) {
                            applySystemRowGlass(v, "reconfig");
                            if (viewSetGlassRadius != null && isRowBackground(v)) {
                                try {
                                    viewSetGlassRadius.invoke(v,
    ...
    private void applySharedMaterialDelta(float[] params) {
        params[32] += deltaIor;                             // refraction
        params[7] = Math.max(0.0f, params[7] + deltaDarker);   // darkening
        params[35] = Math.max(0.0f, params[35] + deltaBurn);   // burn
        params[6] += deltaBrightness;                       // brightness
        params[5] += deltaSaturation;                       // saturation
        params[14] = Math.max(0.0f, params[14] + deltaAlpha);// alpha
        params[11] += deltaTintR;                           // tint R
        params[12] += deltaTintG;                           // tint G
        params[13] += deltaTintB;                           // tint B
        params[21] += deltaEdgeThickness;                   // edge thickness
        params[24] += deltaReflectStrength;                 // reflection strength
        params[28] += deltaLightIntensity;                  // directional light
        params[33] += deltaBgSat;                           // bg saturation
        params[34] += deltaBgBright;                        // bg brightness
    }
    ...
    /** Heads-up notification glass array (alpha=1.5, reflect offset 600). */
    private static boolean isHeadsUpArray(float[] p) {
        return Math.abs(p[14] - 1.5f) < 0.01f
                && Math.abs(p[22] - 600.0f) < 0.01f;
    }

    /** Absolute tuned recipe: stock normal-notification array + shared delta. */
    private float[] buildNotificationRecipe() {
    ...
    /** Idempotency check against the current tuned recipe (values depend on
     *  the runtime config, so this must be dynamic). */
    private boolean isNotificationRecipe(float[] p) {
        return Math.abs(p[6] - (-0.02f + deltaBrightness)) < 0.001f
                && Math.abs(p[7] - (0.30f + deltaDarker)) < 0.001f
                && Math.abs(p[32] - (4.0f + deltaIor)) < 0.001f
                && Math.abs(p[35] - (0.0f + deltaBurn)) < 0.001f
                && Math.abs(p[14] - (0.10f + deltaAlpha)) < 0.001f;
    }
}

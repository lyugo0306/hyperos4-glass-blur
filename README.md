...
- 🔔 **悬浮通知玻璃统一**：悬浮通知（heads-up）系统使用专属玻璃参数（透明度/颜色与列表不同），点击会跳变；模块统一为列表参数，弹出/点击全程一致
...
产物输出到 `release/os4-glass-blur-v6.1.apk`（已签名，使用 debug keystore）。

## 目录结构

```
├── app/
│   ├── AndroidManifest.xml      # 模块清单（versionCode 35 / 6.1）
│   ├── res/                     # 资源（应用名等）
